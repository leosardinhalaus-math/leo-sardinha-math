...
if 'import { getCardImage } from "@/data/cardArt";' not in text:
    text='import { getCardImage } from "@/data/cardArt";\n'+text
...
new_enemy="{enemyUnits.map((unit) => <div key={unit.id} className=\"unit-token enemy-token\" style={{ left: `${82 - unit.progress * 0.64}%`, borderColor: unit.color, boxShadow: `0 0 18px ${unit.color}` }}><img className=\"unit-avatar\" src={getCardImage(unit.cardId)} alt={unit.name} /><span>{unit.name}</span></div>)}"
...
new_ally="{units.map((unit) => <div key={unit.id} className=\"unit-token\" style={{ left: `${18 + unit.progress * 0.64}%`, borderColor: unit.color, boxShadow: `0 0 18px ${unit.color}` }}><img className=\"unit-avatar\" src={getCardImage(unit.cardId)} alt={unit.name} /><span>{unit.name}</span></div>)}"
