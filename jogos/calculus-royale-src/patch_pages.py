...
if 'import { getCardSpriteStyle } from "@/data/cardSprite";' not in text:
    text='import { getCardSpriteStyle } from "@/data/cardSprite";\n'+text
...
old_enemy="{enemyUnits.map((unit) => <div key={unit.id} className=\"unit-token enemy-token\" style={{ left: `${82 - unit.progress * 0.64}%`, borderColor: unit.color, boxShadow: `0 0 18px ${unit.color}` }}><img src={getCardArt(allCards.find((card) => card.id === unit.cardId) ?? cumulativeCards[0])} alt=\"\" /><span>{unit.name}</span></div>)}"
new_enemy="{enemyUnits.map((unit) => <div key={unit.id} className=\"unit-token enemy-token\" style={{ left: `${82 - unit.progress * 0.64}%`, borderColor: unit.color, boxShadow: `0 0 18px ${unit.color}` }}>{getCardSpriteStyle(unit.cardId) ? <div className=\"unit-avatar\" style={getCardSpriteStyle(unit.cardId)} role=\"img\" aria-label={unit.name} /> : <img className=\"unit-avatar-fallback\" src={getCardArt(allCards.find((card) => card.id === unit.cardId) ?? cumulativeCards[0])} alt=\"\" />}<span>{unit.name}</span></div>)}"
old_ally="{units.map((unit) => <div key={unit.id} className=\"unit-token\" style={{ left: `${18 + unit.progress * 0.64}%`, borderColor: unit.color, boxShadow: `0 0 18px ${unit.color}` }}><img src={getCardArt(allCards.find((card) => card.id === unit.cardId) ?? cumulativeCards[0])} alt=\"\" /><span>{unit.name}</span></div>)}"
new_ally="{units.map((unit) => <div key={unit.id} className=\"unit-token\" style={{ left: `${18 + unit.progress * 0.64}%`, borderColor: unit.color, boxShadow: `0 0 18px ${unit.color}` }}>{getCardSpriteStyle(unit.cardId) ? <div className=\"unit-avatar\" style={getCardSpriteStyle(unit.cardId)} role=\"img\" aria-label={unit.name} /> : <img className=\"unit-avatar-fallback\" src={getCardArt(allCards.find((card) => card.id === unit.cardId) ?? cumulativeCards[0])} alt=\"\" />}<span>{unit.name}</span></div>)}"
if old_enemy not in text or old_ally not in text:
