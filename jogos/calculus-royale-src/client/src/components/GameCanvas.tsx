...
type View = "home" | "map" | "battle" | "deck";
...
export default function GameCanvas() {
  ...
  const selectWorld = (index: number) => {
    ...
    const nextIds = Array.from(new Set([...mainDeckIds, ...getCumulativeCards(index).map((card) => card.id)]));
  ...
  const requestSurvivalChallenge = () => {
    ...
    const questions: Challenge[] = [
      { type: "limit", question: `Onda ${wave}: qual é a derivada de x³?`, options: ["3x²", "x²", "3x"], answer: "3x²" },
      { type: "root", question: `Onda ${wave}: qual é ∫₀¹ 3x² dx?`, options: ["1", "3", "1/3"], answer: "1" },
      { type: "zoom", question: `Onda ${wave}: para f(x)=x², onde f'(x)=0?`, options: ["x=0", "x=1", "x=2"], answer: "x=0" },
    ];
    const difficulty = Math.min(2, Math.floor(Math.max(1, wave) / 4));
    setSurvivalChallenge(questions[(wave + difficulty) % questions.length]);
  };
  ...
  return (
    <main className={`game-shell screen-${view}`} style={{ "--accent": WORLD_MAPS[worldIndex].accent, "--glow": WORLD_MAPS[worldIndex].accent, backgroundImage: `linear-gradient(rgba(7, 19, 41, .78), rgba(7, 19, 41, .94)), url(${currentWorldArt})` } as React.CSSProperties}>
        ...
        <div className="content-grid">
          ...
          <section className="battle-column">
            ...
            <div className="arena-frame" data-world={WORLD_MAPS[worldIndex].slug}>
              <WorldArena worldIndex={worldIndex} active={view === "battle" && !survivalOpen && !inventoryOpen} />
              ...
              <div className="battle-field"><div className="lane-line left" /><div className="lane-line right" /><div className="bridge-line" /><div className="tower-tile enemy-tile left-tower"><span>∫</span></div><div className="tower-tile enemy-tile right-tower"><span>lim</span></div><div className="tower-tile ally-tile left-tower"><span>f′</span></div><div className="tower-tile ally-tile right-tower"><span>Σ</span></div>{enemyUnits.map((unit) => <div key={unit.id} className={`unit-token enemy-token ${unit.progress >= 82 ? "is-attacking" : "is-moving"}`} style={{ left: `${82 - unit.progress * 0.64}%`, borderColor: unit.color, boxShadow: `0 0 18px ${unit.color}` }}><img className="unit-avatar" src={getCardImage(unit.cardId)} alt={unit.name} onError={(event) => { const image=event.currentTarget; if(image.dataset.fallbackApplied==="1") return; image.dataset.fallbackApplied="1"; image.src=getCardImageFallback(unit.cardId); }} /><span>{unit.name}</span></div>)}{units.map((unit) => <div key={unit.id} className={`unit-token ${unit.progress >= 82 ? "is-attacking" : "is-moving"}`} style={{ left: `${18 + unit.progress * 0.64}%`, borderColor: unit.color, boxShadow: `0 0 18px ${unit.color}` }}><img className="unit-avatar" src={getCardImage(unit.cardId)} alt={unit.name} onError={(event) => { const image=event.currentTarget; if(image.dataset.fallbackApplied==="1") return; image.dataset.fallbackApplied="1"; image.src=getCardImageFallback(unit.cardId); }} /><span>{unit.name}</span></div>)}{units.length === 0 && enemyUnits.length === 0 && <div className="battle-hint"><Sparkles size={18} /><span>jogue uma carta<br /><small>e veja a prova avançar</small></span></div>}</div>
