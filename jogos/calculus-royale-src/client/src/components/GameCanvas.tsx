...
type Unit = { id: number; icon: string; name: string; kind: Card["kind"]; progress: number; power: number; color: string };
type Challenge = { type: "limit" | "root" | "zoom"; question: string; options: string[]; answer: string };
type View = "map" | "battle" | "deck";
...
const worlds: World[] = [
  {
    ...
    cards: [
      { id: "slope", name: "Cavaleiro da Tangente", icon: "↗", kind: "tropa", cost: 3, rarity: "comum", formula: "f'(a) = lim [f(a+h)−f(a)] / h", effect: "Avança velozmente e perfura uma torre.", power: 14 },
  ...
  },
  {
    ...
    cards: [
      { id: "chain", name: "Arqueira da Cadeia", icon: "∘", kind: "tropa", cost: 3, rarity: "rara", formula: "(f∘g)' = f'(g) · g'", effect: "Cada acerto dispara uma flecha interna.", power: 16 },
      ...
      { id: "rate", name: "Relógio de Taxas", icon: "dt", kind: "feitiço", cost: 4, rarity: "rara", formula: "dx/dt · dy/dx = dy/dt", effect: "Acelera todas as tropas no caminho.", power: 18 },
    ],
  },
  {
    id: 3,
    ...
    cards: [
      ...
      { id: "taylor", name: "Bardo de Taylor", icon: "Σ", kind: "tropa", cost: 3, rarity: "épica", formula: "f(x) ≈ Σ f⁽ⁿ⁾(a)(x−a)ⁿ/n!", effect: "Replica o último ataque com erro mínimo.", power: 22 },
      { id: "newton", name: "Cavaleiro Newton", icon: "N", kind: "tropa", cost: 5, rarity: "lendária", formula: "xₙ₊₁ = xₙ − f(xₙ)/f'(xₙ)", effect: "Salta para a raiz mais próxima da torre.", power: 30 },
  ...
  },
  {
    ...
    cards: [
      { id: "area", name: "Arquiteta da Área", icon: "∫", kind: "tropa", cost: 3, rarity: "comum", formula: "∫ₐᵇ f(x)dx = área líquida", effect: "Recupera vida proporcional ao terreno.", power: 15 },
      { id: "ftc", name: "Oráculo Fundamental", icon: "F", kind: "feitiço", cost: 4, rarity: "épica", formula: "d/dx ∫ₐˣ f(t)dt = f(x)", effect: "Converte acúmulo em um raio imediato.", power: 24 },
...
const extraCards: Record<number, Card[]> = {
  ...
  5: [
    ...
    { id: "series", name: "Mergulhadora de Séries", icon: "Σ∞", kind: "tropa", cost: 5, rarity: "lendária", formula: "Σ aₙ converge?", effect: "Mergulha no infinito e retorna com combo.", power: 30 },
...
const worldArt = [
  `${assetBase}assets/art/world-1.svg`,
  `${assetBase}assets/art/world-2.svg`,
  `${assetBase}assets/art/world-3.svg`,
  `${assetBase}assets/art/world-4.svg`,
  `${assetBase}assets/art/world-5.svg`,
];
...
export default function GameCanvas() {
  ...
  const [activeTab, setActiveTab] = useState<"arena" | "deck">("arena");
  ...
  useEffect(() => {
    ...
    const aiTimer = window.setInterval(() => {
      ...
      setEnemyEnergy((value) => {
        ...
        setEnemyUnits((current) => [...current, { id: Date.now(), icon: aiCard.icon, name: aiCard.name, kind: aiCard.kind, progress: 0, power: Math.max(5, Math.round(aiCard.power * .55)), color: "#ff7d72" }]);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      if (tick % 3 === 0) {
        ...
        setSurvivalEnemies((current) => [...current, { id: Date.now(), icon: card.icon, name: card.name, kind: card.kind, progress: 0, power: Math.max(4, Math.round(card.power * (.34 + wave * .018))), color: "#ff7d72" }]);
  ...
  const playCard = (card: Card) => {
    ...
    setUnits((value) => [...value, { id, icon: card.icon, name: card.name, kind: card.kind, progress: 0, power: Math.round(card.power * (1 + level * .12)), color: world.color }]);
  ...
  const requestPowerUp = (type: "limit" | "root" | "zoom") => {
    const advanced = worldIndex >= 3;
    const questions: Record<Challenge["type"], Challenge> = {
      limit: advanced ? { type: "limit", question: "Qual é ∫₀¹ 2x dx?", options: ["1", "2", "1/2"], answer: "1" } : { type: "limit", question: "Qual é a derivada de x²?", options: ["2x", "x", "x²"], answer: "2x" },
      root: advanced ? { type: "root", question: "Qual é limₙ→∞ (1 + 1/n)ⁿ?", options: ["e", "1", "∞"], answer: "e" } : { type: "root", question: "Qual é uma primitiva de 2x?", options: ["x²", "2x²", "ln x"], answer: "x²" },
      zoom: advanced ? { type: "zoom", question: "O método de Simpson aproxima uma integral usando...", options: ["parábolas", "retas apenas", "limites laterais"], answer: "parábolas" } : { type: "zoom", question: "Se f'(x) > 0, a função está...", options: ["crescendo", "decrescendo", "constante"], answer: "crescendo" },
    };
    setChallenge(questions[type]);
  };
  ...
  const evolveCard = (card: Card) => {
    ...
    const level = cardLevels[card.id] ?? 0;
    const price = level === 0 ? 2 : 4;
    if (essence < price) { setToast("Essência insuficiente para esta evolução."); return; }
    setEssence((value) => value - price); setCardLevels((levels) => ({ ...levels, [card.id]: level + 1 })); setToast(`${card.name} evoluiu para nível ${level + 1}.`);
  };

  const selectWorld = (index: number) => {
    const unlocked = index === 0 || completedWorlds[index - 1];
    if (!unlocked) { setToast("Conclua a ilha anterior para liberar esta fase."); return; }
    const nextIds = mainDeckIds.filter((id) => getCumulativeCards(index).some((card) => card.id === id));
    if (!nextIds.length) nextIds.push(getCumulativeCards(index)[0].id);
    setWorldIndex(index);
    setChapter(0);
    ...
    setToast(`Mundo ${index + 1} desbloqueado · ${nextIds.length} cartas no deck`);
  };
  ...
  const startSurvival = () => { setSurvivalOpen(true); setSurvivalRunning(false); setSurvivalTower(100); setSurvivalEnemies([]); setSurvivalWave(0); setSurvivalScore(0); setSurvivalElixir(7); survivalTickRef.current = 0; survivalWaveRef.current = 0; survivalScoreRef.current = 0; survivalEndedRef.current = false; setSurvivalRewardNotice(null); setSurvivalChallenge(null); };
  const requestSurvivalChallenge = () => {
    const level = survivalWaveRef.current;
    const pool = level < 5 ? [
      { type: "limit" as const, question: "Qual é a derivada de x³?", options: ["3x²", "x²", "3x"], answer: "3x²" },
      { type: "root" as const, question: "∫₀¹ 3x² dx vale...", options: ["1", "3", "1/3"], answer: "1" },
    ] : [
      { type: "zoom" as const, question: "Se f''(x) < 0, o gráfico é...", options: ["côncavo para baixo", "côncavo para cima", "constante"], answer: "côncavo para baixo" },
      { type: "limit" as const, question: "A série geométrica converge quando |r| é...", options: ["< 1", "> 1", "= 2"], answer: "< 1" },
      { type: "root" as const, question: "O Teorema Fundamental liga derivada e...", options: ["integral", "matriz", "vetor unitário"], answer: "integral" },
    ];
    setSurvivalChallenge(pool[level % pool.length]);
    setSurvivalChallengeWave(level);
  };
  ...
  return (
    <main className="game-shell" style={{ "--accent": world.color, "--glow": world.glow, backgroundImage: `linear-gradient(rgba(7, 19, 41, .78), rgba(7, 19, 41, .94)), url(${currentWorldArt})` } as React.CSSProperties}>
      <a className="site-back-link" href="../../">← Leo Sardinha.Math</a>
      ...
      <section className="hud">
        ...
        <div className="content-grid">
          <aside className="left-panel panel-glass">
            ...
            <div className="world-showcase"><div className="section-label">ARTE DA COLEÇÃO</div><img src={currentWorldArt} alt={`Painel ilustrado de ${world.title}`} /><small>Os assets ilustrados deste mundo agora fazem parte do projeto e guiam a identidade visual da fase.</small></div>
          ...
          <section className="battle-column">
            <div className="battle-status"><div className="status-pill"><span className="live-dot" /> BATALHA AO VIVO</div><span className="arena-caption"><Swords size={14} /> Arena {world.id === 4 ? "Delta" : "de Estudos"}</span><span className="ai-chip"><span className="ai-pulse" /> IA · {enemyEnergy.toFixed(1)} elixir</span><button className="icon-button" onClick={resetBattle} title="Reiniciar batalha"><RotateCcw size={15} /></button></div>
            <div className="arena-frame">
              ...
              <div className="battle-field"><div className="lane-line left" /><div className="lane-line right" /><div className="bridge-line" /><div className="tower-tile enemy-tile left-tower"><span>∫</span></div><div className="tower-tile enemy-tile right-tower"><span>lim</span></div><div className="tower-tile ally-tile left-tower"><span>f′</span></div><div className="tower-tile ally-tile right-tower"><span>Σ</span></div>{enemyUnits.map((unit) => <div key={unit.id} className="unit-token enemy-token" style={{ left: `${82 - unit.progress * 0.64}%`, borderColor: unit.color, boxShadow: `0 0 18px ${unit.color}` }}><b>{unit.icon}</b><span>{unit.name}</span></div>)}{units.map((unit) => <div key={unit.id} className="unit-token" style={{ left: `${18 + unit.progress * 0.64}%`, borderColor: unit.color, boxShadow: `0 0 18px ${unit.color}` }}><b>{unit.icon}</b><span>{unit.name}</span></div>)}{units.length === 0 && enemyUnits.length === 0 && <div className="battle-hint"><Sparkles size={18} /><span>jogue uma carta<br /><small>e veja a prova avançar</small></span></div>}</div>
      ...
      {challenge && <div className="challenge-backdrop"><div className="challenge-modal"><div className="challenge-icon">?</div><span className="section-label">DESAFIO-RELÂMPAGO</span><h2>Resolva para ativar o power-up</h2><p>{challenge.question}</p><div className="challenge-options">{challenge.options.map((option) => <button key={option} onClick={() => answerChallenge(option)}>{option}</button>)}</div><small>Resposta rápida mantém sua sequência de batalha.</small></div></div>}
